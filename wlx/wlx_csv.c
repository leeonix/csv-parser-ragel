...
//
//       Filename:  wlx_csv.c
//         Author:  LeeoNix (Refactored for Ragel 2.0 Parser)
//        Created:  2015-6-23 16:51:23 (Updated 2026)
//
//    Description: Total Commander Lister Plugin for CSV (Powered by High-Performance Ragel)
//
// =====================================================================================
...
static void listview_copy_selected()
{
    ...
    if (index >= 0) {
        ...
        if (buf_len > 0 && field_buf[buf_len - 1] == ',') {
...
/*
 * =====================================================================================
 * [核心重构] 适配新 Ragel 解析器的单元格回调函数
 * =====================================================================================
 * 新解析器直接赋予了绝对的 row_idx (从0开始) 和 col_idx (从0开始)，以及脱壳洗净后的文本。
 * 彻底移除了老版判断 TK_EOL / TK_String / TK_Quote 的繁琐分支，实现极速 UI 组装！
 * =====================================================================================
 */
static void read_csv_cell(void *ctx, int row_idx, int col_idx, const char *field, size_t field_len)
{
    /* [C89 规范] 严格于块顶部声明局部变量 */
    char buf[16];

    /* 1. 动态表头自适应扩展：只要遇到了新的最大列号，立刻向 ListView 追加列！
     * 解决了老代码只在第一行 (record_no == 1) 能够建列的局限性 */
    if (col_idx >= column_count) {
        sprintf(buf, "%d", col_idx);
        listview_add_column(hListView, col_idx, cal_width((int)field_len), buf);
    }

    /* 2. 遇到了每行的第 0 列时，代表新行发车，向 ListView 插入 Item */
    if (col_idx == 0) {
        listview_insert_item(hListView, row_idx);
    }

    /* 3. 动态计算并撑开列宽 */
    adjust_column_width((int)field_len, col_idx);

    /* 4. 填充该行该列的单元格文本 (内部执行 UTF-8 -> ANSI/ACP 转换) */
    listview_set_item(hListView, row_idx, col_idx, get_field_buf(field, (int)field_len));
}
...
HWND __stdcall ListLoad(HWND ParentWin, char *FileToLoad, int ShowFlags)
{
    ...
    if (hwnd != NULL) {
        ...
        /* [核心调用升级] 使用新接口一次性无损加载并解析物理文件 */
        csv_parse_file(FileToLoad, read_csv_cell, NULL);
